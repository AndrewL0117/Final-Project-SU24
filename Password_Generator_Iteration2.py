...
def generate_password(length=12, include_digits=True, digits_min=2, include_special_chars=True, special_chars_min=1):
    ...
    chars = list(string.ascii_letters)  # Letters
    if include_digits:
        chars.extend(string.digits)  # Add digits
    if include_special_chars:
        chars.extend(string.punctuation)  # Add special characters

    # Ensure password has min. required numbers and special characters
    password = []
    if include_digits:
        password.extend(random.choice(string.digits) for _ in range(digits_min))
    if include_special_chars:
        password.extend(random.choice(string.punctuation) for _ in range(special_chars_min))
